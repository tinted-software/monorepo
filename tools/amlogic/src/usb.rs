...
impl UsbDevice {
    ...
    pub fn open(vid: u16, pid: u16, timeout: Duration) -> Result<Self> {
    ...
    fn open_matched(info: &nusb::DeviceInfo) -> Result<Self> {
        let device = info
            .open()
            .wait()
            .map_err(|e| report!("failed to open device: {e}"))?;
        ...
        if device.active_configuration().is_err() {
            device
                .set_configuration(1)
                .wait()
                .map_err(|e| report!("failed to set configuration: {e}"))?;
        }

        let interface = device
            .claim_interface(0)
            .wait()
            .map_err(|e| report!("failed to claim interface 0: {e}"))?;

        let (out_ep, in_ep) = probe_bulk_endpoints(&interface).unwrap_or((0x01, 0x81));
        let out_ep_handle = interface
            .endpoint::<Bulk, Out>(out_ep)
            .map_err(|e| report!("failed to open bulk OUT endpoint {out_ep:#04x}: {e}"))?;
        let in_ep_handle = interface
            .endpoint::<Bulk, In>(in_ep)
            .map_err(|e| report!("failed to open bulk IN endpoint {in_ep:#04x}: {e}"))?;
    ...
    pub fn control_transfer(
        ...
        if request_type & 0x80 != 0 {
            let resp = self
                ...
                .wait()
                .map_err(|e| report!("control IN transfer failed: {e}"))?;
            ...
            self.interface
                ...
                .wait()
                .map_err(|e| report!("control OUT transfer failed: {e}"))?;
            Ok(len)
        }
    }

    /// Performs a USB bulk write to the given endpoint.
    pub fn bulk_write(&self, _ep: u8, data: &[u8], timeout_ms: u32) -> Result<usize> {
        let len = data.len();
        ...
        .into_result()
        .map_err(|e| report!("bulk write failed: {e}"))?;
        Ok(len)
    }

    /// Performs a USB bulk read from the given endpoint.
    pub fn bulk_read(&self, _ep: u8, buf: &mut [u8], timeout_ms: u32) -> Result<usize> {
        ...
        let received = completion
            .into_result()
            .map_err(|e| report!("bulk read failed: {e}"))?;
    ...
    pub fn reset(&self) -> Result<()> {
        ...
        bail!("USB port reset is not supported by this tool")
    }
}
