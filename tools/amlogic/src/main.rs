...
use amlogic_boot_protocol::{AmlogicDevice, SocId};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Ramboot {
            ...
            // Read Image file
            let mut img_file = File::open(&image)
                .map_err(|e| report!("Failed to open image '{}': {}", image.display(), e))?;
            ...
            let has_dtb = if let Some(ref dtb_path) = dtb {
                let mut dtb_file = File::open(dtb_path)
                    .map_err(|e| report!("Failed to open DTB '{}': {}", dtb_path.display(), e))?;
        ...
        Commands::WriteMem {
            ...
            let mut f = File::open(&file)
                .map_err(|e| report!("Failed to open file '{}': {}", file.display(), e))?;
        ...
        Commands::ReadMem { addr, len, out } => {
            ...
            let mut f = File::create(&out)
                .map_err(|e| report!("Failed to create output file '{}': {}", out.display(), e))?;
            f.write_all(&data)?;
        ...
        Commands::BootG12 { uboot } => {
            let mut f = File::open(&uboot).map_err(|e| {
                report!("Failed to open u-boot binary '{}': {}", uboot.display(), e)
            })?;
            let mut data = Vec::new();
            f.read_to_end(&mut data)?;
