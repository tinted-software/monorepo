...
python3 "$root/apply_fixups.py" "$kdk_kernel" "$out_dir/xnu.fixed.macho"

echo "==> Neutralizing arm64e PAC instructions (no FEAT_PAuth on cortex-a76 either)..."
python3 "$root/patch_pac.py" "$out_dir/xnu.fixed.macho" "$out_dir/xnu.macho"

echo "==> Laying out kernel segments for physical load..."
prep_out=$(python3 "$root/prepare_guest.py" "$out_dir/xnu.macho" "$out_dir/xnu.bin")
echo "$prep_out"
...
echo "==> Building hv for QEMU virt via Bazel (board::DRAM_BASE=$dram_base)..."
bazel build //boot/hv:run_qemu

extra_dev=(-device "loader,file=$out_dir/xnu.bin,addr=$guest_entry_pa,force-raw=on")
exec bazel run //boot/hv:run_qemu -- "${extra_dev[@]}" "$@"
